...
namespace WpfViewer.Models
{
    ...
    public class RenderModel : Livet.NotificationObject
    {
        ...
        Dictionary<Node<NodeContent>, Mesh> m_meshMap = new Dictionary<Node<NodeContent>, Mesh>();
        ...
        IEnumerable<IRenderCommand> Commands
        {
            get
            {
                ...
                foreach (var mesh in m_scene.Root
                    .Traverse()
                    .Where(x => m_meshMap.ContainsKey((Node<NodeContent>)x))
                    .Select(x => m_meshMap[(Node<NodeContent>)x])
        ...
        Mesh AddModel(Node<NodeContent> model)
        {
